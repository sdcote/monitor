...
/**
 * Provides an implementation for NTLMv1, NTLMv2, and NTLM2 Session forms of 
...
public class NTLMHandlerImpl implements NTLMHandler {
  ...
  static {
    final byte[] bytesWithoutNull = getBytes( "NTLMSSP", DEFAULT_CHARSET );
   ...
   * @param domain The domain with which to authenticate.
   * 
   ...
   * @throws NTLMEngineException If {@code encrypt(byte[],byte[])} fails.
   */
  ...
  static class NTLMMessage {
    ...
    NTLMMessage( final String messageBody, final int expectedType ) throws NTLMEngineException {
      messageContents = fromBase64( getBytes( messageBody, DEFAULT_CHARSET ) );
    ...
    protected int readUShort( final int position ) throws NTLMEngineException {
      return NTLMHandlerImpl.readUShort( messageContents, position );
    }
    ...
    protected int readULong( final int position ) throws NTLMEngineException {
      return NTLMHandlerImpl.readULong( messageContents, position );
    }
    ...
    protected byte[] readSecurityBuffer( final int position ) throws NTLMEngineException {
      return NTLMHandlerImpl.readSecurityBuffer( messageContents, position );
    }
     ...
     *
     * @param maxlength the maximum length of the response to prepare, not including the type and the signature (which this method adds).
     */
    ...
    /**
     * Returns the response that has been generated after shrinking the array 
     * if required and base64 encodes the response.
     *
    ...
    String getResponse() {
      ...
      return getAsciiString( toBase64( resp ) );
    }
    ...
    private byte[] toBase64( byte[] resp ) {
    ...
    private byte[] fromBase64( byte[] bytes ) {
      // TODO Auto-generated method stub
      return null;
    }
  ...
  static class Type1Message extends NTLMMessage {
    ...
    Type1Message( final String domain, final String host ) throws NTLMEngineException {
      ...
      try {
        ...
        domainBytes = unqualifiedDomain != null ? unqualifiedDomain.toUpperCase( Locale.US ).getBytes( "ASCII" ) : null;
      } catch ( final UnsupportedEncodingException e ) {
  ...
  static class Type3Message extends NTLMMessage {
    ...
    Type3Message( final String domain, final String host, final String user, final String password, final byte[] nonce, final int type2Flags, final String target, final byte[] targetInformation ) throws NTLMEngineException {
      ...
      try {
        ...
        domainBytes = unqualifiedDomain != null ? unqualifiedDomain.toUpperCase( Locale.US ).getBytes( "UnicodeLittleUnmarked" ) : null;
  ...
  public static byte[] getBytes( final String data, final String charset ) {
    Assert.notNull( data, "Input" );
    Assert.notEmpty( charset, "Charset" );
    try {
